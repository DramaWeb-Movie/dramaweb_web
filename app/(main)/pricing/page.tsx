  ...
  return (
    <div className="min-h-screen bg-gray-50 pt-24">
      <div className="container mx-auto px-4 md:px-8 py-12">
        ...
        <div className={`grid gap-8 max-w-5xl mx-auto ${plans.length === 1 ? 'max-w-sm' : plans.length === 2 ? 'md:grid-cols-2 max-w-3xl' : 'md:grid-cols-3'}`}>
          {plans.map((plan, idx) => {
              ...
              >
                ...
                <div className="p-6 border-b border-gray-100">
                  ...
                  <h2 className="text-2xl font-bold text-gray-900 mb-2">
                    {plan.name} <span className="text-sm font-normal text-gray-400 capitalize">({plan.billing_period})</span>
                  </h2>
                ...
                <div className="p-6 flex-1 flex flex-col">
                  <div className="flex items-baseline gap-1 mb-1">
                    <span className="text-4xl font-bold text-[#E31837]">
                      {plan.currency === 'USD' ? '$' : plan.currency}{price.toFixed(2)}
                    </span>
          ...
          {plans.length === 0 && (
            <div className="bg-white rounded-2xl border-2 border-[#E31837]/50 overflow-hidden flex flex-col relative shadow-sm">
              <div className="absolute top-0 right-0 bg-[#E31837] text-white text-xs font-semibold px-4 py-1.5 rounded-bl-xl">
                {t('popular')}
              </div>
              <div className="p-6 border-b border-gray-100">
                ...
                <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('fallbackName')}</h2>
                <p className="text-gray-500 text-sm">{t('fallbackDesc')}</p>
              </div>
              <div className="p-6 flex-1 flex flex-col">
                <div className="flex items-baseline gap-1 mb-1">
                ...
                <p className="text-gray-400 text-sm mb-6">{t('fallbackBilled')}</p>
                <ul className="space-y-3 mb-8">
                  <li className="flex items-center gap-2 text-gray-600 text-sm">
                    <FiCheck className="text-[#E31837] shrink-0" /> {t('allSeries')}
                  </li>
                ...
                <Link href="/series" className="mt-auto">
                  <Button className="w-full flex items-center justify-center gap-2">
                    <FiDollarSign className="text-lg" /> {t('subscribeNow')}
                  </Button>
                </Link>
