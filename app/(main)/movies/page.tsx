...
export default function MoviesPage() {
  ...
  const [items, setItems] = useState<{ id: string; title: string; titleKh?: string; episodes: number; image: string; price?: number }[]>([]);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setLoading(true);
      try {
        const [moviesRes, supabase] = await Promise.all([
          fetch('/api/movies?type=single', { signal: ac.signal }),
          (async () => createClient())(),
        ]);
        ...
        if (user) {
          ...
          const ids = new Set((purchaseRows || []).map((r: { content_id: string }) => r.content_id));
          setPurchasedMovieIds(ids);
        }
      } catch (err) {
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 md:gap-6">
            {paginatedItems.map((item) => (
              <DramaCardCompact
                ...
                image={item.image}
                showMovieButton
                hasPurchased={purchasedMovieIds.has(item.id)}
                price={item.price}
              />
            ))}
          </div>
